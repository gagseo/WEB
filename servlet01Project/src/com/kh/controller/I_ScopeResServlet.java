...
public class I_ScopeResServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		int sessionSum = (session == null) ? 0 : (int) (session.getAttribute("sessionSum"));
		...
		pw.println("<h4>각 페이지간의 정보가 전달되는 범위를 말한다.<br>" + "이동하는 페이지 마다 생성되고 사라진다.</h4>");
		pw.println("계산결과 : " + reqSum);
		pw.println("<br><hr>");

		pw.println("<h2>Session 영역</h2>");
		pw.println("<h4>웹브라우저를 연결하는 동안 객체를 생성하여 유지하며<br>" + "연결이 종료되면 사라진다.</h4>");
		pw.println("계산결과 : " + sessionSum);
		pw.println("<br><hr>");

		pw.println("<h2>Application 영역</h2>");
		pw.println("<h4>서버가 실행되어 있는 동안에 객체를 유지하는 범위<br>" + "서버가 실행되어 있는 동안 계속 정보를 유지한다.</h4>");
