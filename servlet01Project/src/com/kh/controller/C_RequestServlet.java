...
public class C_RequestServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		// ex) 브라우저 정보, 클라이언트 시스템, 쿠키, 세션, 사용자가 전송한 데이터 등등....
		...
		response.getWriter().println("<h5>이름 : " + request.getParameter("name") + "</h5>");
	...
	private void responseUrlencoded(HttpServletRequest request, HttpServletResponse response)
		...
		// 본래 application/x-www-urlencoded 컨텐츠 타입이
		// 데이터를 어떤 형태로 보내는지 확인해보기.
		PrintWriter pw = response.getWriter();

		/*
		 * int data = 0; String origin = "";
		 * 
		 * while((data = request.getInputStream().read()) != -1){ origin += (char)data;
		 * }
		 */
		// pw.println("<h3>origin : " + origin + "</h3>");

		pw.println("<h1>Request 객체</h1>");
		...
		pw.println("<h5>서버이름 : " + request.getServerName() + "</h5>");
		pw.println("<h5>요청한 URI 경로 : " + request.getRequestURI() + "</h5>");
		pw.println("<h5>ContextPath : " + request.getContextPath() + "</h5>");
		pw.println("<h5>이름 : " + request.getParameter("name") + "</h5>");
		pw.println("<h5>나이 : " + request.getParameter("age") + "</h5>");
		// request.getParameter() 사용하기
		// name이랑 age 찍어보기

		// request.getParameterMap() 매서드 써보기
	...
	private void responseMulti(HttpServletRequest request, HttpServletResponse response)
		...
		pw.println("<h5>요청한 URI 경로 : " + request.getRequestURI() + "</h5>");
		pw.println("<h5>ContextPath : " + request.getContextPath() + "</h5>");
		pw.println("<h5>이름 : " + request.getParameter("text") + "</h5>");
